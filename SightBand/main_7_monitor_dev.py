...
with dai.Device(pipeline) as device:
    ...
    def getWarning(xmin,ymin,xmax,ymax,depth):
        ...
        med       = np.percentile(depth_arr, 70)
        focal_length_in_pixels = 400 * 0.5 / math.tan(HFOV * 0.5 * math.pi / 180)
        real_depth = int(focal_length_in_pixels * BASELINE / med) if med!=0 else 500
        if real_depth < 60:
            warning = 'Dangerous'
    ...
    while True:
        ...
        if inDisparity is not None:
            ...
            cropped = depth_frame[40:360,180:500]
            depth_scaled  = cv2.resize(cropped, None, fx= 1.3, fy= 1.3, interpolation= cv2.INTER_LINEAR)
