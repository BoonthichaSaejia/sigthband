...
with dai.Device(pipeline) as device:
    ...
    def displayFrame(name, frame):
        ...
        for detection in detections:
            ...
            cv2.putText(frame, f"{int(detection.confidence)}%", (bbox[0] + 10, bbox[1] + 40), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0,0,255))
            cv2.putText(frame, labelLocation(bbox[2],bbox[0]), (bbox[0] + 10, bbox[1] + 60), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0,255,0))
            cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, 2)
            print("we could detect", labelMap[detection.label], "confidence=",int(detection.confidence),'\%')
            vibrate(labelLocation(bbox[2],bbox[0]))
                        
        # Show the frame
        cv2.imshow(name, frame)
        #flipped_frame = cv2.flip(frame,-1)
        #cv2.imshow(name, flipped_frame)
