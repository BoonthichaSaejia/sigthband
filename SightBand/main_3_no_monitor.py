...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        def labelLocation(j):
            if j==0:
                return 'left'
            elif j==width_section-1:
                return 'right'
            elif j==width_section//2:
                return 'center'
            else:
                return 'void'
        ...
        for i in range(3):
            ...
            for j in range(width_section):
                if warning_section[i][j] > max_num[j]:
                    max_num[j] = warning_section[i][j]
        for i in range(width_section):
            if max_num[i] >= 60 and (max_num[i]<=160):
                vibrate(labelLocation(i),1)
            elif (max_num[i] <60) and (max_num[i]>=0):
